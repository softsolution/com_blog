...
if ($do=='config'){
    ...
    if (!cmsCore::inRequest('goadd')){
        ...
        cmsPage::initTemplate('components', 'com_blog_config')->
                assign('blog', $blog)->
                assign('form_action', '/blog/'.$blog['id'].'/editblog.html')->
    ...
    if (cmsCore::inRequest('goadd')){
        ...
        //Проверяем настройки
        if (mb_strlen($title)<3) { $title = $blog['title']; }
...
if ($do=='view'){
    ...
    $sql = "SELECT b.*, p.imageurl 
        FROM cms_blogs b 
        LEFT JOIN cms_user_profiles p ON p.user_id = b.user_id 
        WHERE b.id = '{$blog_id}' AND b.owner = '{$inBlog->owner}' 
        ORDER BY b.id ASC LIMIT 1";

    $result = $inDB->query($sql);
    if ($inDB->num_rows($result)){
        $blog = $inDB->fetch_assoc($result);
    }
    ...
    if(isset($blog['imageurl'])){
            $blog['author_avatar'] = cmsUser::getUserAvatarUrl($blog['user_id'], 'small', $blog['imageurl'], 0);
    } else {
        $blog['author_avatar'] = "/images/users/avatars/small/nopic.jpg";
    }
	...
	$inPage->addHead('<link rel="alternate" type="application/rss+xml" title="'.htmlspecialchars(strip_tags($blog['title'])).'" href="'.HOST.'/rss/blog/'.$blog['id'].'/feed.rss">');
    ...
    $posts = $inBlog->getPosts(($inUser->is_admin || $myblog), $model, true);
    if(!$posts && $page > 1){ cmsCore::error404(); }
    
    $month_full_str   = array(''.$_LANG['MONTH_01_SHORT'].'', ''.$_LANG['MONTH_02_SHORT'].'', ''.$_LANG['MONTH_03_SHORT'].'', ''.$_LANG['MONTH_04_SHORT'].'', ''.$_LANG['MONTH_05_SHORT'].'', ''.$_LANG['MONTH_06_SHORT'].'', ''.$_LANG['MONTH_07_SHORT'].'', ''.$_LANG['MONTH_08_SHORT'].'', ''.$_LANG['MONTH_09_SHORT'].'', ''.$_LANG['MONTH_10_SHORT'].'', ''.$_LANG['MONTH_11_SHORT'].'', ''.$_LANG['MONTH_12_SHORT'].'');
    $month_int        = array('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12');
    
    foreach($posts as $key=>$post){
        $date_time = explode(' ', $post['pubdate']);
        $date = explode('-', $date_time[0]);
        $posts[$key]['number'] = $date[2];
        $posts[$key]['month_name'] = str_replace($month_int, $month_full_str, $date[1]);
        $posts[$key]['tagline']    = getTagLine('blogpost', $post['id']);
    }
    ...
    if($blogcats && $cat_id){
        foreach($blogcats as $blogcat){
            if($blogcat['id']==$cat_id){
                $cur_desc = $blogcat['description'];
                $cur_title = $blogcat['title'];
            }
        }
    }
    ...
    ob_start();
    $inPage->printModules('blog');
    $blog_modules = ob_get_clean();
    ...
    cmsPage::initTemplate('components', 'com_blog_view')->
        ...
        assign('blog', $blog)->
        assign('posts', $posts)->
        assign('cur_desc', $cur_desc)->
        assign('cur_title', $cur_title)->
        assign('pagination', $pagination)->
        assign('blog_modules', $blog_modules)->     
...
if ($do=='newpost' || $do=='editpost'){
    ...
    if (!cmsCore::inRequest('goadd')){
        ...
        $bb_toolbar = cmsPage::getBBCodeToolbar('message',$model->config['img_on'], 'blog', 'post', $post_id);
...
if ($do=='newcat' || $do=='editcat'){
    ...
    if (!cmsCore::inRequest('goadd')){
        ...
        cmsPage::initTemplate('components', 'com_blog_edit_cat.tpl')->
        assign('mod', $cat)->
        assign('form_action', ($do=='newcat' ? '/blog/'.$blog['id'].'/newcat.html' : '/blog/editcat'.$cat['id'].'.html'))->
...
if($do=='post'){
    ...
    $post['tags']    = getTagLine('blogpost', $post['id']);
    $post['pubdate'] = date("d/m/Y", strtotime($post['pubdate']));
    ...
    if($inCore->isComponentInstalled('comments') && $post['comments']){
        ...
        $inPage->comments_global = ob_get_clean();
        $inPage->commens_on = $post['comments'];
    }
...
////////// VIEW POPULAR POSTS ////////////////////////////////////////////////////////////////////////////////////////
    if ($do=='best'){
        ...
        $inDB->limitPage($page, $model->config['perpage']);
            ...
            assign('posts', $posts)->
            assign('pagination', cmsPage::getPagebar($total, $page, $model->config['perpage'], '/blog/popular-%page%.html'))->
            assign('cfg', $model->config)->
...
function getTagLine($target, $item_id, $links=true, $selected=''){

    $inDB = cmsDatabase::getInstance();
    $sql  = "SELECT tag FROM cms_tags WHERE target='$target' AND item_id='$item_id' ORDER BY tag DESC";
    $rs = $inDB->query($sql);
    ...
    if ($tags){
        ...
        while ($tag=$inDB->fetch_assoc($rs)){
            if ($links){
                $html .= '<li><a href="/search/tag/'.urlencode($tag['tag']).'"><i class="fa fa-tag"></i>'.$tag['tag'].'</a></li> ';
            } else {
                $html .= $tag['tag'];
            }
        }
