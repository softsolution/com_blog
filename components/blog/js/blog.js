$(function(){
  blog = {
    editBlog: function(blog_id) {
      ...
      $.post('/blog/'+blog_id+'/editblog.html', {}, function(data){
		if(data.error == false){
    ...
    addBlogCat: function(blog_id) {
      ...
      $.post('/blog/'+blog_id+'/newcat.html', {}, function(data){
		if(data.error == false){
			$('#popup_message').html(data.html);
			$('#popup_progress').hide();
    ...
    editBlogCat: function(cat_id) {
      ...
      $.post('/blog/editcat'+cat_id+'.html', {}, function(data){
		if(data.error == false){
			$('#popup_message').html(data.html);
			$('#popup_progress').hide();
			$('#popup_ok').val('Сохранить').show();
    ...
    deleteCat: function(cat_id, csrf_token) {
		core.confirm('Вы действительно хотите удалить эту рубрику блога? Записи рубрики удалены не будут.', null, function(){
			$.post('/blog/delcat'+cat_id+'.html', {csrf_token: csrf_token}, function(result){
				if(result.error == false){
					window.location.href = result.redirect;
				}
			}, 'json');
    ...
    deletePost: function(post_id, csrf_token) {
		core.confirm('Вы действительно хотите удалить этот пост блога?', null, function(){
			$.post('/blog/delpost'+post_id+'.html', {csrf_token: csrf_token}, function(result){
				if(result.error == false){
					window.location.href = result.redirect;
				}
			}, 'json');
    ...
    publishPost: function(post_id) {
      $.post('/blog/publishpost'+post_id+'.html', {}, function(data){
		if(data == 'ok'){
			$('#pub_link').hide();
			$('#pub_wait').hide();
			$('#pub_date').fadeIn();
